...
export class KanjiImporter {
  ...
  private prepareStatements(): void {
    ...
    this.insertKanjiStmt = conn.prepare(`
      INSERT INTO kanji (character, jis_code, unicode, on_readings, kun_readings, meanings, grade, stroke_count, frequency, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
    `);
  }
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        try {
          ...
          this.insertKanjiStmt.run(
            ...
            kanjiEntry.grade ?? null,
            kanjiEntry.strokeCount ?? null,
            kanjiEntry.frequency ?? null
          );
