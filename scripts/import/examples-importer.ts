...
export class ExamplesImporter {
  ...
  private static PARTICLES = new Set(['は', 'を', 'が', 'の', 'に', 'で', 'と', 'も', 'か', 'へ', 'よ', 'ね', 'な', 'わ']);

  /**
   * Parse B: line tokens into a JSON array of dictionary forms.
   * Token format: `食べる(たべる)[01]{食べている}` — dictionary form is text before `(`
   * Tokens without `(` are used as-is (e.g. single kanji or kana words).
   */
  private parseTokens(line: string, lineNumber: number): string | null {
    ...
    for (const token of parts) {
      ...
      const form = parenIdx > 0 ? token.substring(0, parenIdx) : token.split(/[(\[{]/)[0];
      if (form && !ExamplesImporter.PARTICLES.has(form)) {
        forms.push(form);
      }
    }
  ...
  async import(filePath: string): Promise<void> {
    ...
    await this.linkExamplesToWords();

    this.printStats(processedEntries);
  }

  /**
   * Post-insert pass: link examples to words by matching JSON token array against words.word column.
   * Links each example to the first significant (non-particle) match found.
   */
  private async linkExamplesToWords(): Promise<void> {
    console.log('\n🔗 Linking examples to words...');

    const conn = this.db.getConnection();

    // Build a lookup map of word text -> word id
    const wordRows = conn.prepare('SELECT id, word FROM words').all() as Array<{ id: number; word: string }>;
    const wordMap = new Map<string, number>();
    for (const row of wordRows) {
      if (row.word && !wordMap.has(row.word)) {
        wordMap.set(row.word, row.id);
      }
    }
    console.log(`   Loaded ${wordMap.size.toLocaleString()} unique words for matching`);

    // Iterate through all examples and try to match tokens
    const examples = conn.prepare('SELECT id, tokens FROM examples WHERE tokens IS NOT NULL').all() as Array<{ id: number; tokens: string }>;

    let linked = 0;
    const updateStmt = conn.prepare('UPDATE examples SET word_id = ? WHERE id = ?');

    conn.exec('BEGIN TRANSACTION');

    for (const ex of examples) {
      try {
        const forms: string[] = JSON.parse(ex.tokens);
        for (const form of forms) {
          const wordId = wordMap.get(form);
          if (wordId !== undefined) {
            updateStmt.run(wordId, ex.id);
            linked++;
            break;
          }
        }
    ...
    console.log(`   Linked ${linked.toLocaleString()} of ${total.toLocaleString()} examples (${pct}%)`);
  }
